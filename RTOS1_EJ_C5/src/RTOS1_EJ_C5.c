...
void tecla_led_init(void){
	...
	for(i = 0 ; i < N_TECLAS ; i++){
		...
		teclaLED[i].semaphore = xSemaphoreCreateCounting(MAX_SEMPH, INICIO_SEMPH);
...
void tarea_led( void* taskParmPtr )
{
	...
	TickType_t xPeriodicity =  2*LED_TIME;		// Tarea periodica cada 1000 ms
